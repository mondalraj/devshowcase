        ...
        if (token) {
            try {
                ...
                const comment = new Comment({
                  ...
                  const project = await Project.findByIdAndUpdate({ _id: project_id }, {
                    $push: {
                ...
                res.status(201).json({ status: "success", user_id:user_id.id, project: project, comment:projectComment, message: 'Comment Successfully added' });
            } catch (error) {
                res.status(201).json({ status: "fail", message: 'User not Authorized', error: error.message });
            }
